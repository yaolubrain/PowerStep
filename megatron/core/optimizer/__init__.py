...
def _get_megatron_optimizer_based_on_param_groups(
    ...
    elif config.optimizer == 'powerstep':
        optimizer = PowerStep(
            param_groups,
            lr=config.lr,
            gamma=config.powerstep_gamma,
            beta=config.powerstep_beta,
            weight_decay=config.weight_decay
        )
        init_state_fn = None

    elif config.optimizer == 'powerstep_int8':
        optimizer = PowerStepInt8(
            param_groups,
            lr=config.lr,
            gamma=config.powerstep_gamma,
            beta=config.powerstep_beta,
